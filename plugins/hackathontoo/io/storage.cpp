...
namespace hackathon
{
  ...
  bool Storage::writeCache(const Cache &cache)
  {
    ...
    query.addBindValue(cache.colorArgb);
    query.addBindValue(cache.colorA);
    query.addBindValue(cache.colorR);
    query.addBindValue(cache.colorG);
    query.addBindValue(cache.colorB);
